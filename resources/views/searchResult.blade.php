          ...
          <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  ...
                  <div class="p-3 md:p-4 rounded-3xl border border-zinc-100 hover:shadow-lg transition relative flex flex-col">
                      ...
                      <a href="{{ route('product.details',['id'=>$data->id, 'slug'=>$data->slug]) }}" class="mb-4 block">
                          <img class="h-40 mx-auto object-contain"
                               src="{{ asset('storage/' . ($data->images ? explode('-', $data->images)[0] : 'defaults/no-image.png')) }}"
                               alt="{{ $data->title }}" />
                      </a>

                      {{-- نام محصول --}}
                      <a href="{{ route('product.details',['id'=>$data->id, 'slug'=>$data->slug]) }}" class="text-xs md:text-sm font-semibold text-zinc-700 line-clamp-2 mb-3">
                          {{ $data->name }}
                      </a>
                      ...
                      <div class="mt-auto flex justify-between items-center">
                          <a href="{{ route('product.details',['id'=>$data->id, 'slug'=>$data->slug]) }}" class="bg-primary-500 hover:bg-primary-400 px-3 py-2 rounded-xl shadow-lg transition">
              ...
              <span class="text-xl md:text-2xl text-red-600 font-bold">
                ...
                {{ number_format($data->price) }}
                              ...
                              @else
                                  <span class="text-xl md:text-2xl text-zinc-800 font-semibold">
              {{ number_format($data->price) }}
            </span>
                              @endif
