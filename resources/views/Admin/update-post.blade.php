...
<div class="flex h-screen">
    ...
    <main class="flex-1 p-8 overflow-y-auto">
        ...
        <div class="bg-white p-8 rounded-3xl shadow-lg hover-scale fade-in mb-8">
            <h2 class="text-3xl font-bold text-gray-800 mb-6">بروزرسانی</h2>
            ...
            <form action="{{ route('admin.posts.update', $post->id) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="mb-6">
                    ...
                    <textarea  name="description" class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:ring-2
                     focus:ring-indigo-600 focus:border-transparent transition duration-200">{{ old('description', $post->description) }}</textarea>
                    @error('description')
                    <div class="text-red-600 text-sm mt-2">{{ $message }}</div>
                    @enderror
                </div>
                <div class="mb-6">
                    ...
                    <textarea id="content" name="body" class="w-full px-4 py-2 rounded-lg border border-gray-300
                    focus:ring-2 focus:ring-indigo-600 focus:border-transparent transition duration-200">{{ old('body', $post->body) }}</textarea>
                    @error('body')
                    <div class="text-red-600 text-sm mt-2">{{ $message }}</div>
                    @enderror
                </div>
                <div class="mb-6">
                    ...
                    @if($post->image)
                        <img src="{{ asset('storage/' . $post->image) }}" alt="تصویر پست" class="w-32 h-32 mb-4 rounded-lg">
                    @endif
                ...
                <div class="mb-6">
                    ...
                    <select id="category" name="category"
                        ...
                        <option value="blog" {{ old('category', $post->category) == 'blog' ? 'selected' : '' }}>وبلاگ</option>
                        <option value="service" {{ old('category', $post->category) == 'service' ? 'selected' : '' }}>خدمات</option>
                        <option value="worksample" {{ old('category', $post->category) == 'worksample' ? 'selected' : '' }}>نمونه کار</option>
                    </select>
                    @error('category')
                    <div class="text-red-600 text-sm mt-2">{{ $message }}</div>
                    @enderror
                ...
                <div class="mb-6">
                    <label for="tags" class="block text-sm font-medium text-gray-700 mb-2">برچسب‌ها (با کاما جدا کنید)</label>
