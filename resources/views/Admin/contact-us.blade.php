            ...
            <div class="overflow-x-auto">
                <table class="w-full border-collapse text-gray-800">
                    ...
                    <tr class="bg-gray-50">
                        ...
                        <th class="p-3 text-right">عملیات</th> {{-- ستون جدید برای دکمه حذف --}}
                    </tr>
                    </thead>
                    <tbody>
                    ...
                    <span class="truncate w-40 inline-block"
                          onmouseover="showTooltip(event, '{{ $data->message }}')"
                          onmousemove="moveTooltip(event)"
                          onmouseout="hideTooltip()">
                        {{ \Illuminate\Support\Str::limit($data->message, 40, '...') }}
                    </span>
                            </td>
                            ...
                            <td class="p-3 text-center">{{ $data->number }}</td>
                            <td class="p-3 text-center">{{ $data->name }}</td>

                            <td class="p-3 text-center">
                                <form action="{{ route('contactus.destroy', $data->id) }}" method="POST" onsubmit="return confirm('آیا مطمئن هستید که می‌خواهید حذف کنید؟');">
                                    @csrf

                                    <button type="submit" class="bg-red-500 hover:bg-red-600 text-white text-sm px-4 py-2 rounded-lg shadow">
                                        حذف
                                    </button>
                                </form>
                            </td>
