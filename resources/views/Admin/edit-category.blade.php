        ...
        <section class="max-w-4xl mx-auto glass-effect p-6 md:p-8 rounded-3xl shadow-2xl fade-in">
            <div class="text-center mb-8">
                <div class="inline-flex items-center justify-center w-16 h-16 bg-blue-100 rounded-full mb-4">
                    <i class="fas fa-pencil-alt text-blue-600 text-2xl"></i> </div>
                <h2 class="text-3xl font-bold text-gray-800 mb-2">ویرایش دسته‌بندی</h2> <p class="text-gray-600">اطلاعات دسته‌بندی را ویرایش و به‌روزرسانی کنید</p> </div>
            ...
            <form action="{{ route('admin.category.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                @csrf
                 <div class="group">
                    ...
                    <input type="text" name="title" id="categoryTitle"
                           ...
                           value="{{ old('title', $category->title) }}"> @error('title')
                ...
                <div class="group">
                    ...
                    <input type="text" name="name" id="categoryName"
                           ...
                           onchange="updatePreview()" required
                           value="{{ old('name', $category->name) }}"> <p class="text-sm text-gray-500 mt-1">نام انگلیسی برای URL استفاده می‌شود (مثل: electronics)</p>
                    @error('name')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="group">
                    ...
                    <textarea name="description" id="categoryDescription"
                              ...
                              onchange="updatePreview()">{{ old('description', $category->description) }}</textarea> @error('description')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="group">
                    ...
                    <div class="flex items-center gap-4 mb-4">
                        <div class="icon-preview" id="iconPreview">
                            <i class="{{ old('icon', $category->icon) ?? 'fas fa-question text-2xl text-gray-400' }}"></i>
                        </div>
                        <div>
                            <p class="font-medium text-gray-700">آیکن انتخابی</p>
                            <p class="text-sm text-gray-500">برای تغییر، روی آیکن مورد نظر کلیک کنید</p>
                        </div>
                    </div>

                    <div class="icon-selector" id="iconSelector">
                        <div class="icon-item" onclick="selectIcon('fas fa-plug', this)">...</div>
                        <div class="icon-item" onclick="selectIcon('fas fa-lightbulb', this)">...</div>
                    </div>
                ...
                <div class="group">
                    ...
                    <div class="upload-area border-2 border-dashed border-gray-300 rounded-xl p-8 text-center cursor-pointer"
                        ...
                        <div class="mb-4">
                            ...
                            <p class="text-gray-600 font-medium">برای **جایگزینی** تصویر، آن را اینجا بکشید یا کلیک کنید</p>
                    ...
                    <p class="text-sm text-gray-500 mt-2">
                        ...
                        اگر تصویری انتخاب نکنید، تصویر فعلی باقی می‌ماند.
                    </p>
                    @error('image')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                    ...
                    <div id="imagePreview" class="mt-6 {{ $category->image ? '' : 'hidden' }}">
                        <div class="image-preview relative inline-block">
                            <img id="previewImg" src="{{ $category->image ? asset('storage/' . $category->image) : '/placeholder.svg' }}" alt="پیش‌نمایش" class="w-48 h-32 object-cover rounded-lg">
                ...
                <div class="group">
                    ...
                    <div class="category-preview" id="categoryPreview">
                        <div class="flex items-center justify-center mb-3">
                            <i id="previewIcon" class="{{ old('icon', $category->icon) ?? 'fas fa-question text-3xl' }}"></i>
                        </div>
                        <h3 id="previewTitle" class="text-xl font-bold mb-2">{{ old('title', $category->title) ?? 'عنوان دسته‌بندی' }}</h3>
                        <p id="previewDescription" class="text-sm opacity-90">{{ old('description', $category->description) ?? 'توضیحات دسته‌بندی اینجا نمایش داده می‌شود' }}</p>
                    </div>
                </div>
                ...
                <div class="text-center pt-6">
                    <button type="submit"
                        ...
                        <i class="fas fa-sync-alt ml-2"></i> به‌روزرسانی دسته‌بندی </button>
                </div>
            </form>
        </section>
