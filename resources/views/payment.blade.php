...
<head>
    ...
    <title>فروشگاه آنلاین نلی گالری</title>
</head>
...
<main class="mt-0 md:mt-8">
    <nav class="flex items-center gap-1 md:gap-5 w-full md:w-10/12 md:mx-auto">
        ...
        <a class="flex flex-col md:flex-row items-center w-fit min-w-24 md:min-w-32 gap-1 text-green-500 text-sm" href="cart.blade.php">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path class="stroke-green-600" d="M11 20H10.5C6.741 20 4.862 20 3.607 19.01C3.40756 18.852 3.2222 18.677 3.053 18.487C2 17.307 2 15.537 2 12C2 8.463 2 6.694 3.053 5.513C3.22167 5.32367 3.40633 5.14967 3.607 4.991C4.862 4 6.741 4 10.5 4H13.5C17.259 4 19.138 4 20.392 4.99C20.5933 5.15 20.7783 5.32433 20.947 5.513C21.896 6.577 21.99 8.118 22 11M2 9H22M14 18C14 18 15 18 16 20C16 20 19.177 15 22 14" stroke="black" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
            اتمام خرید
        </a>
    </nav>
    ...
    <div class="mt-10 pb-20">
        <div class="bg-zinc-100 {{ $order->status == 'paid' ? 'text-green-600' : 'text-red-600' }} text-base md:text-xl py-5 text-center leading-8">
            @if($order->status == 'paid')
                با تشکر از خرید شما
                <br>
                خرید با موفقیت انجام شد. کد پیگیری: {{ $refId }}
            @else
        ...
        <div class="bg-zinc-100 grid grid-cols-2 gap-y-7 md:grid-cols-4 justify-items-center px-4 md:px-20 py-5 mt-8">
            <div class="flex flex-col justify-start gap-1">
                <div class="text-xs text-zinc-800 font-yekanBakhRegular">شماره سفارش:</div>
                <div class="text-sm text-zinc-800 font-yekanBakhBold">{{ $order->tracking_code }}</div>
            </div>
            <div class="flex flex-col justify-start gap-1">
                <div class="text-xs text-zinc-800 font-yekanBakhRegular">تاریخ:</div>
                <div class="text-sm text-zinc-800 font-yekanBakhBold">
                    {{ \Morilog\Jalali\Jalalian::fromDateTime($order->created_at)->format('Y/m/d') }}
                </div>
            </div>
            <div class="flex flex-col justify-start gap-1">
                <div class="text-xs text-zinc-800 font-yekanBakhRegular">قیمت تمام شده:</div>
                <div class="text-sm text-zinc-800 font-yekanBakhBold">{{ number_format($order->total_price) }} تومان</div>
            </div>
            <div class="flex flex-col justify-start gap-1">
                <div class="text-xs text-zinc-800 font-yekanBakhRegular">روش پرداخت:</div>
                <div class="text-sm text-zinc-800 font-yekanBakhBold">درگاه زرین‌پال</div>
            </div>
        </div>

        <div class="bg-zinc-100 py-5 mt-10 px-4 md:px-10">
            <p class="text-zinc-800 text-lg mb-8">جزئیات پرداخت</p>
            <table class="text-zinc-700 bg-white text-xs rounded-lg w-full md:w-10/12 mx-auto">
                <tbody class="divide-y divide-zinc-300">
                @foreach($order->items as $item)
                    <tr>
                        <td class="px-4 py-3">{{ $item->product->title }} ({{ $item->quantity }} عدد)</td>
                        <td class="font-yekanBakhRegular">{{ number_format($item->product->price * $item->quantity) }} تومان</td>
                    </tr>
                @endforeach
                <tr>
                    <td class="px-4 py-3">هزینه ارسال:</td>
                    <td class="font-yekanBakhRegular">
                        {{ $order->shipping_cost > 0 ? number_format($order->shipping_cost) . ' تومان' : 'رایگان / پس‌کرایه' }}
                    </td>
                </tr>
                <tr class="bg-zinc-50 font-bold">
                    <td class="px-4 py-3">مجموع نهایی:</td>
                    <td class="font-yekanBakhRegular">{{ number_format($order->total_price) }} تومان</td>
                </tr>
                </tbody>
            </table>
        </div>
        ...
        @if(is_array($address))
            <div class="bg-zinc-100 py-5 mt-8 px-4 md:px-10">
                <p class="text-zinc-800 text-lg mb-8">جزئیات ارسال</p>
...
{{--                        <td class="font-yekanBakhRegular">--}}
{{--                            @php--}}
{{--                                $city = new App\Models\ProvinceCities;--}}
{{--                            @endphp--}}
{{--                            {{$city->cityName($address['province']) ?? '-' }}، {{ $city->cityName($address['city']) ?? '-' }}--}}
{{--                        </td>--}}
                    </tr>

                    <tr>
                        <td class="px-4 py-3">نشانی:</td>
                        <td class="font-yekanBakhRegular">
