...
{{--                </div>--}}
{{--            </div>--}}
            <div class="px-2 md:px-3 pt-2 pb-5 mb-3 text-zinc-700 text-sm md:text-base flex items-center gap-x-2 border-b border-zinc-200">
                <svg class="fill-primary-500" xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="" viewBox="0 0 256 256"><path d="M128,16a88.1,88.1,0,0,0-88,88c0,75.3,80,132.17,83.41,134.55a8,8,0,0,0,9.18,0C136,236.17,216,179.3,216,104A88.1,88.1,0,0,0,128,16Zm0,56a32,32,0,1,1-32,32A32,32,0,0,1,128,72Z"></path></svg>
                آخرین سفارشات
            </div>
            <div
                ...
                <table class="w-full text-right table-auto min-w-max">
                    ...
                    @foreach(auth()->user()->orders as $item)
                        ...
                        <td class="p-4">
                            <p class="block text-sm">
                               @php
                                   $orderDate = Jalalian::fromDateTime($item->created_at)->format('Y/m/d');
                               @endphp
                        ...
                        <td class="p-4">
                            <a href="{{ route('dashboard.orders.details', $item->id) }}" class="block text-xs">مشاهده</a>
                        </td>
                    </tr>
                    @endforeach
                    </tbody>
